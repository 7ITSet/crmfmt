...
jQuery.fn.sug = function(o) {
	...
	var o = $.extend({
		// URL для поиска слов
		url:'/ajax/suggest_services_products.php',
		...
		tableValue:'m_orders_smeta_table[]',
		productsValue:'m_orders_smeta_products',
		servicesValue:'m_orders_smeta_services',
		...
		dataSend:function(input) {  
			return 'w='+word+'&products='+input.parents('form:first').find("input[name='"+o.productsValue+"']").prop('checked')+'&services='+input.parents('form:first').find("input[name='"+o.servicesValue+"']").prop('checked');
		},
		...
		wordClick:function(input,link){
			...
			input.parents('div.row:first').find("input[name='"+o.tableValue+"']").val(table);
		...
		.keydown(function(e) {
			...
			if ($('.suggest:visible').length){
				...
				if (e.keyCode == 38 || e.keyCode == 40) {
					...
					table=suggest.children('a.suggest-selected').attr('data-table');
					input.parents('div.row:first').find("input[name="+o.idValue+"]").val(id);
					input.parents('div.row:first').find("input[name="+o.categoryValue+"]").val(category);
					input.parents('div.row:first').find("input[name="+o.unitValue+"]").val(unit);
					input.parents('div.row:first').find("input[name="+o.priceValue+"]").val(price);
