...
jQuery.fn.sug = function(o) {
	...
	var o = $.extend({  
		// URL для поиска слов
		url:'/ajax/suggest_bik.php',
		//поле для подстановки значения
		ksValue:'m_companies_bank_ks',
		nameValue:'m_companies_bank_name',
		...
		wordClick:function(input,link){
			...
			ks=link.attr('rel');
			$("input[name="+o.ksValue+"]").val(ks);
			name=link.text();
			$("input[name="+o.nameValue+"]").val(name);
		...
		.keydown(function(e) {  
			...
			if ($('.suggest:visible').length){
				...
				if (e.keyCode == 38 || e.keyCode == 40) {
					...
					ks=suggest.children('a.suggest-selected').attr('rel');
					$('input[name='+ksValue+']').val(ks);
					name=suggest.children('a.suggest-selected').text();
