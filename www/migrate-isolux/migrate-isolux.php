...
if($res_islx)
	foreach($res_islx as &$_item){
		...
		$q='SELECT `m_products_id`,`m_products_price_general`,`m_products_check_it`,`m_products_docs`,`m_products_foto` FROM `m_products` WHERE 
		...
		if(!$res=$sql->query($q)){
			...
			if($product_cat=addCategory(implode('|',$_item['category']))){
				...
				if($_item['links']){
					...
					$q='SELECT `m_products_id` FROM `m_products` WHERE `m_products_id_isolux` IN ('.implode(',',$_item['links']).');';
					if($res=$sql->query($q))
						foreach($res as $_res)
				...
				if($sql->query($q)){
					...
					if($_item['foto']){
						...
						if($product_foto){	
							...
							$q='UPDATE `m_products` SET `m_products_foto`=\''.$product_foto.'\' WHERE `m_products_id`=\''.$product_id.'\' LIMIT 1;';
						}
						//не показываем товары, у которых нет фото
						else{
							$q='UPDATE `m_products` SET `m_products_show_site`=0 WHERE `m_products_id`='.$product_id.' LIMIT 1;';
						}
						$sql->query($q);
					}
					...
					if($sql->query($q)){
						...
						if($update_foto||!$res[0]['m_products_foto']){
							if($_item['foto']){
								...
								if($product_foto){	
									$product_foto=json_encode($product_foto,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
									$q='UPDATE `m_products` SET `m_products_show_site`=1,`m_products_check_it`=0,`m_products_foto`=\''.$product_foto.'\' WHERE `m_products_id`=\''.$product_id.'\' LIMIT 1;';
								}
								//не показываем товары, у которых нет фото
								else{
									$q='UPDATE `m_products` SET `m_products_show_site`=0 WHERE `m_products_id`='.$product_id.' LIMIT 1;';
...
function addCategory($_cat){
	...
	for($i=0;$i<sizeof($_cat);$i++){
		...
		for($j=$i;$j>=0;$j--)
			$q.='SELECT `m_products_categories_id` FROM `m_products_categories` WHERE 
