...
?>	
			...
			<div id="userinfo">
				...
				<a href="javascript:void(0);" id="userinfo-name"
		...
		$(document).ready(function() {
			...
			$('#userinfo-name').on('click',function(){
				...
				$('#userinfo-detail-info').parents('.popover').css({'left':userinfodetail.left-144,'top':'+=5'});
			});
			...
			$('.datatable').each(function(index,el){
				...
				var lengthMenu=[[20, 50, 100, -1], [20, 50, 100, "Все"]];
				...
				if($(this).find('th.order').length)
					var otable=$(this).DataTable({
						...
						"columnDefs": [{"targets":no_order,"orderable":false}],
				...
				if(isNaN(localStorage.getItem("row_count"))||localStorage.getItem("row_count")===null) {}
				else{$(this).DataTable().page.len(localStorage.getItem("row_count")*1).draw();}
			});
