	...
	foreach($res as $_item){
		...
		if($_item['m_products_docs']){
			...
			foreach($_item['m_products_docs'] as $_doc){
				$file=$_SERVER['DOCUMENT_ROOT'].'/../../p-islx.Formetoo.ru/documents/'.substr($_item['m_products_id_isolux'],0,2).'/SN'.$_item['m_products_id_isolux'].'/'.$_doc->url;
				if(file_exists($file)){
					...
					//если файла нет - добаляем его в базу и записываем ID для товара
						...
						$q='INSERT INTO `formetoo_main`.`m_products_docs` SET 
							...
							`m_products_docs_filedir`='.$_item['m_products_id_isolux'].',
...
echo $q.'<br/>';
			...
			$q='UPDATE `formetoo_main`.`m_products` SET 
				...
				WHERE `m_products_id`='.$_item['m_products_id'].' LIMIT 1;';
echo $q.'<br/>';				
			if($sql->query($q))
				$result[]=$_item['m_products_id'];
		}
	}
...
if($result){
	...
	foreach($result as $_result)
		echo '&nbsp;&nbsp;'.$res[$_result][0]['m_products_id'].' '.$res[$_result][0]['m_products_name_full'].'<br/>';
	}
else
