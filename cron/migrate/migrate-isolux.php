...
$length=get('length')?get('length'):100;
$update_foto=false;
$update_docs=false;
$update_attr=true;
$update_text=true;
...
$q='SELECT * FROM `p-islx`.`ci_goods` WHERE `isload`=1 AND `price`!=\'0\' AND `url`!=\'\' ORDER BY `date` DESC LIMIT '.$start.','.$length.';';
...
if($res_islx)
	foreach($res_islx as &$_item){
		...
		$_item['links']=explode('|',$_item['links']);
		...
		if(!$res=$sql->query($q)){
			...
			if($product_cat=addCategory(implode('|',$_item['category']))){
				$attributes=$sql->query('SELECT * FROM `formetoo_main`.`m_products_attributes_list`;','m_products_attributes_list_name');
				...
				if($sql->query($q)){
					...
					//ДОБАВЛЯЕМ ТЕКСТ
					$q='INSERT INTO `formetoo_main`.`m_products_desc` SET 
					...
					if($_item['attributes'])
						foreach($_item['attributes'] as $_attr_item){
							...
							if(!isset($attributes[$_a_n])||(isset($attributes[$_a_n])&&!in_array($attr_type,$_a_types))){
								...
								$q='INSERT INTO `formetoo_main`.`m_products_attributes_list` SET 
							...
							elseif($attr_type==1){
								...
								$sql->query('INSERT INTO `formetoo_main`.`m_products_attributes_values` SET
									`m_products_attributes_values_id`='.$attr_value.',
									`m_products_attributes_values_value`=\''.$_a_v.'\';');
							}
							$q='INSERT INTO `formetoo_main`.`m_products_attributes` SET 
			...
			if($res[0]['m_products_check_it']){
				...
				if($product_cat=addCategory(implode('|',$_item['category']))){
					...
					if($update_docs||!$res[0]['m_products_docs']){
						$res[0]['m_products_docs']='0|'.$res[0]['m_products_docs'];
						$q='SELECT * FROM `formetoo_main`.`m_products_docs` WHERE 
						...
						if($res_d=$sql->query($q)){
							...
							$sql->query('DELETE `formetoo_main`.`m_products_docs` WHERE `m_products_docs_id` IN ('.str_replace('|',',',$res[0]['m_products_docs']).');');
						}
						$docs=array();
						foreach($_item['docs'] as $_doc){
							$file=__DIR__.'/../../../p-islx.Formetoo.ru/documents/'.substr($_item['id'],0,2).'/SN'.$_item['id'].'/'.$_doc->url;
					...
					if($sql->query($q)){
						...
						if($update_attr){
							if($_item['attributes']){
								$sql->query('DELETE FROM `formetoo_main`.`m_products_attributes` WHERE `m_products_attributes_product_id`='.$product_id.';');
								foreach($_item['attributes'] as $_attr_item){
...
echo '<a href="http://cron.formetoo.ru/migrate/migrate-isolux.php?start='.($start+$length).'">Продолжить с позиции '.($start+$length).'</a>';
	...
	for($i=0;$i<sizeof($_cat);$i++){
		...
		if(!$res=$sql->query($q)){
			...
			if($sql->query($q)){
				...
				$q='INSERT INTO `formetoo_main`.`menu` SET 
