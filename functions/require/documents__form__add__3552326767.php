...
if(get('m_documents_id')){
	...
	$document['params']->doc_status_otchet=isset($document['params']->doc_status_otchet)?$document['params']->doc_status_otchet:'01';
	//скидка по заказу
	...
	foreach($documents->getInfo() as $_document)
		if($_document[0]['m_documents_order']==$order['m_orders_id']&&$_document[0]['m_documents_templates_id']==3552326767)
			$acts[]=$_document[0];
...
$content->setJS('
 ...
 /* 	$("td.check input").trigger("change"); */
	...
	$("td.check input:checked").trigger("change");
...
<fieldset>
	...
	<div class="row">
		...
		<section class="col col-2">
			<label class="label">Статус в отчете</label>
			<label class="input">
				<input type="text" name="m_documents_status_otchet" placeholder="01" title="Основание" value="<?=$document['params']->doc_status_otchet?>">
			</label>
		</section>
	</div>
<?
...
?>	
									<tr>
										...
										<td class="price">
											...
											<input type="hidden" name="m_orders_act_services_nds[]" value="<?=(isset($_service->nds)?$_service->nds:$order['m_orders_nds'])?>" data-clean="no"/>
										</td>
										<td class="sum">
...
<input type="hidden" name="m_documents_templates_id" value="3552326767"/>
<?
}
														/* НОВЫЙ */
else{
...
<fieldset>
	...
	<div class="row">
		...
		<section class="col col-2">
			<label class="label">Статус в отчете</label>
			<label class="input">
				<input type="text" name="m_documents_status_otchet" placeholder="01" title="Основание">
			</label>
		</section>
	</div>
<?
