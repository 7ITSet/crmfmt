...
if(get('m_documents_id')){
	...
	foreach($documents->getInfo() as $_document)
		if($_document[0]['m_documents_order']==$order['m_orders_id']&&$_document[0]['m_documents_templates_id']==3522102145)
			$acts[]=$_document[0];
	...
	<div class="row">
		...
		<section class="col col-3">
			...
			<label class="checkbox">
				<input type="checkbox" name="m_documents_noprice" <?=(isset($document['params']->noprice)&&$document['params']->noprice?' checked':'')?> value="1"/>
				<i></i>
				Без цен, только объёмы
			</label>
		</section>
	</div>
<?
...
?>	
									<tr>
										...
										<td>
											<?=$services->services_id[$_service->id][0]['m_services_price_general_w']?>
											<input type="hidden" name="m_orders_act_services_price[]" value="<?=$services->services_id[$_service->id][0]['m_services_price_general_w']?>" data-clean="no"/>
										</td>
										<td class="sum">
											<?=round($services->services_id[$_service->id][0]['m_services_price_general_w']*$_service->count,2)?>
											<input type="hidden" name="m_orders_act_services_sum[]" value="<?=round($services->services_id[$_service->id][0]['m_services_price_general_w']*$_service->count,2)?>" data-clean="no"/>
										</td>
									</tr>
<?
						}
...
<input type="hidden" name="m_documents_templates_id" value="3522102145"/>
<?
}
														/* НОВЫЙ */
else{
...
<fieldset>
	<div class="row">
		...
		<section class="col col-3">
			<br/><br/>
			<label class="checkbox">
				<input type="checkbox" name="m_documents_noprice" value="1"/>
				<i></i>
				Без цен, только объёмы
			</label>
		</section>
...
?>								
									<tr>
										...
										<td class="sum">
											<?=$services->services_id[$_service->id][0]['m_services_price_general_w']*$_service->count?>
											<input type="hidden" name="m_orders_act_services_sum[]" value="<?=$services->services_id[$_service->id][0]['m_services_price_general_w']*$_service->count?>" data-clean="no"/>
										</td>
									</tr>
<?
						}
